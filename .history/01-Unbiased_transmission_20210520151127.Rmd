...
この性質を使うと、もっと複雑なプロットを構築することができる。`plot_multiple_runs()`で最初に登場するgeomである`geom_line()`を考えよう。`geom_line()`は最初に呼び出した`ggplot()`で設定したaestheticsを継承するが、加えて`geom_line()`特有の新しいマッピング`colour = run`を指定している。これにより、各試行のラインを違う色で塗るようにggplotに伝えているのだ。次のgeomの`stat_summary()`はすべての試行について平均を計算する
