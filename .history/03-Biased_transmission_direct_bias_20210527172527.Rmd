...
$probability\_a = probability\_b$のとき、バイアスなしのコピーであり、個々人は自らの形質と異なった形質を持つ個人と出会ったときにその形質に持ち替える確率は同じである。この場合、バイアスのない伝達のシミュレーション結果が再現される。とくに$probability\_a = probability\_b = 1$のとき、このモデルは完全に[第一章][バイアスのない伝達]で実装したものと同一になる。
