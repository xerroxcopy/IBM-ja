...
model_unbiased_transmissions <- function(population_size, generations, runs) {
  ...
  for (run in 1:runs) {
    ...
    output_single_run <- tibble(run = rep(run, generations) %>% as.factor(), generation = 1:generations, proportion_of_trait_a = rep(NA, generations))
    # 第1世代
    population <- sample(c("A", "B"), population_size, replace = TRUE)
    ...
    for (generation in 2:generations) {
      ...
      population <- sample(previous_population, population_size, replace = TRUE)
      # proportion_of_trait_aを計算し、tibbleのうちproportion_of_trait_a列のgeneration行目に格納する
      output_single_run$proportion_of_trait_a[generation] <- sum(population == "A") / population_size
    }
...
model_unbiased_transmissions_varying_initial_condition <- function(
  ...
  for (run in 1:runs) {
    # 1回の試行の結果を保管するtibbleを初期化
    output_single_run <- tibble(run = rep(run, generations), generation = 1:generations, proportion_of_trait_a = rep(NA, generations))
    # 第1世代
    population <- sample(c("A", "B"), population_size, replace = TRUE, prob = c(initial_proportion_of_trait_a, 1 - initial_proportion_of_trait_a))
    # 第1世代のproportion_of_trait_aを計算
    proportion_of_trait_a <- sum(population == "A") / population_size
    output_single_run$proportion_of_trait_a[1] <- proportion_of_trait_a
    for (generation in 2:generations) {
