...
続く行`if_else()`を使った行では変異体として選ばれた個人の形質を持ち替えさせている^[原著ではかなり違う方法を使っているが、`if_else()`を用いたほうがより]。
...
といった検討が思い浮かぶ。もしかすると早くなるかもしれないし、遅くなるかもしれない。`bench`ライブラリの`mark()`や`tictoc`ライブラリの`tic()`, `toc()`関数などを活用して、速さを測りながら高速化してみよう。
