...
```{r 3-3, fig.cap = "バイアスのある伝達はs字型の拡散曲線をなす"}
...
（問題3-1）`probability\_b`は$0$のままで、`probability\_a`を$0$から$0.5$まで$0.1$刻みで変えて、それをひとつの図にプロットしてみよう。世代数は変わらず$generations = 150$、試行回数は$runs = 100$

（問題3-2）`probability\_a`と`probability\_b`を$0$から$0.5$まで$0.1$刻みで変えて、それをひとつの図にプロットしてみよう。できれば`pmap()`関数を使いnested tibbleを構築し、それを`unnest()`してからプロットしよう。
